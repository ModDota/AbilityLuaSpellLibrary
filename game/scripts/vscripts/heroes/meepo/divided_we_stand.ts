...
class meepo_divided_we_stand_lua extends CDOTA_Ability_Lua {
  ...
  OnUpgrade() {
    ...
    let mainMeepo:CDOTA_BaseNPC_Hero = PlayerResource.GetSelectedHeroEntity(PID);
    ...
    let newMeepo = <CDOTA_BaseNPC_Hero>CreateUnitByName(caster.GetUnitName(),mainMeepo.GetAbsOrigin(),true,mainMeepo,mainMeepo.GetPlayerOwner(),mainMeepo.GetTeamNumber());
...
class modifier_meepo_divided_we_stand_lua extends CDOTA_Modifier_Lua {
  ...
  OnCreated() {
    if (IsServer()) {
      //this.boots_list = ["item_travel_boots2","item_travel_boots","item_guardian_greaves","item_power_treads","item_arcane_boots","item_phase_boots","item_tranquil_boots","item_boots"]
  ...
  OnIntervalThink() {
    ...
    if (mainMeepo != meepo) {
      let boots:string[] = ["item_travel_boots2","item_travel_boots","item_guardian_greaves","item_power_treads","item_arcane_boots","item_phase_boots","item_tranquil_boots","item_boots"]
  ...
  // Death is dealt with in OnTakeDamge
  /*OnDeath(keys) {
    if (keys.unit != this.GetParent()) {return null}
