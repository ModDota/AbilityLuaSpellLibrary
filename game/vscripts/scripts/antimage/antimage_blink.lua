...
function lua_antimage_blink:OnSpellStart()
    ...
    local ability = self
    local target_position = ability:GetCursorPosition()
    ...
    local min_blink_range = ability:GetSpecialValueFor("min_blink_range")

    -- Get blink distance
    local blink_distance = (target_point - caster:GetAbsOrigin()):Length2D()
    local direction = (target_point - caster:GetAbsOrigin()):Normalized()

    -- Minimum blink distance
    if blink_distance < min_blink_range then
        blink_distance = min_blink_range
    end
